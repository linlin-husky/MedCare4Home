...
function createAuthRoutes(models) {
  ...
  router.post('/register', async (req, res) => {
    try {
      ...
      res.status(500).json({ error: 'server-error', message: 'Internal server error during registration' });
    }
  });
  ...
  router.post('/login', async (req, res) => {
    try {
      ...
      // Handle potential activities error gracefully or check what's wrong
      let unreadCount = 0;
      try {
        unreadCount = await activities.getUnreadCount(username);
      } catch (actErr) {
        console.error('Error fetching unread count:', actErr);
        // Continue login even if activities fail
      }
